...
public class TimesheetProcessingWorkflow
{
    ...
    public async Task RunAsync(TimesheetProcessingInput input)
    {
        ...
        var timesheetId = TimesheetId.New().ToString(); // In real scenario, this would be generated by DB
        await Workflow.ExecuteActivityAsync(
            (ITimesheetProcessingActivities activities) => activities.ValidateTimesheetRulesAsync(new ValidateTimesheetInput(timesheetId)),
            new ActivityOptions { StartToCloseTimeout = TimeSpan.FromMinutes(5) }
        );

        // Step 2: Calculate Payroll and Taxes
        var payrollResult = await Workflow.ExecuteActivityAsync(
            (ITimesheetProcessingActivities activities) => activities.CalculatePayrollAndTaxesAsync(new CalculatePayrollInput(timesheetId)),
            new ActivityOptions { StartToCloseTimeout = TimeSpan.FromMinutes(5) }
        );
        ...
        var paymentReference = await Workflow.ExecuteActivityAsync(
            (ITimesheetProcessingActivities activities) => activities.SubmitBankTransferAsync(new SubmitBankTransferInput(timesheetId, idempotencyKey)),
        ...
        var invoiceNumber = await Workflow.ExecuteActivityAsync(
            (ITimesheetProcessingActivities activities) => activities.GenerateAndSendInvoiceAsync(new GenerateInvoiceInput(timesheetId, netPay.Amount)),
            new ActivityOptions { StartToCloseTimeout = TimeSpan.FromMinutes(5) }
        );
